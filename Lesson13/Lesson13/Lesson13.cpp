...
/*
Операционная система  Windows посылает сообщение WM_COMMAND при 
каждом выборе пункта меню, определяющего команду.  
...
Первый флаг может содержать одно из следующих значений: 
    ...
    •	MF_BYPOSITION – в этом случае второй параметр должен 
        содержать относительную позицию пункта меню с отсчетом от нуля. 
...
Для управления выбором пунктов-переключателей (ставит отметку
на выбранном пункте и снимает со всех остальных пунктов в указанной
группе) используется функция CheckMenuRadioItem(), ее прототип:
...
Задавая пункты меню для группы переключателей, следует
убедиться, что их идентификаторы в файле resource.h
имеют сквозную нумерацию и упорядочены в соответствии 
с позициями этих пунктов на полосе меню.
