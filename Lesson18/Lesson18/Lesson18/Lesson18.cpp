...
TBSTYLE_FLAT		Текст кнопок отображается под картинками.
					Для предотвращаения проблем с перерисовкой, стиль должен быть
					установлен перед отображением панели инструментов.
TBSTYLE_TOOLTIP		Вывод краткого описания кнопки в окне элемента управления Tool Tip
TBSTYLE_LIST		Текст кнопок появляется справа от картинок.
TBSTYLE_TRANSPARENT Создает прозрачную панель инструментов (но не прозрачные кнопки).
...
CCS_ADJUSTABLE		Если указан этот стиль, пользователь может изменять конфигурацию 
					элемента управления
...
Если приложение использует функцию CreateWindowEx для создания панели инструментов, 
...
Идентификаторы кнопок вполне отражают их назначение:
	...
	STD_DELETE,		STD_FILEMEW,	STD_PRINTPRE,	STD_PROPERTIES, STD_HELP, STD_FIND, 
