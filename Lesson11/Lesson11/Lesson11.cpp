...
•	при помощи редактора диалоговых окон на этапе визуально - го проектирования формы диалога;
...
При нажатии одной из стрелок Spin Control посылает своему  роди-тельскому окну сообщение 
WM_VSCROLL или WM_HSCROLL (в зави-симости от ориентации счётчика), в котором младшее 
слово параметра wParam содержит код SB_THUMBPOSITION.  
Кроме того, счетчик по-сылает уведомляющее сообщение UDN_DELTAPOS  в форме сообщения 
WM_NOTIFY.

При непосредственном клавиатурном вводе нового числа в «прия-тельское окно» элемент Edit 
