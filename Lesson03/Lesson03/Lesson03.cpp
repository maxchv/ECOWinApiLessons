...
2)	Почему рекомендуется использование венгерской нотации при име - новании переменных ?
3)	Чем отличается спецификация __cdecl от __stdcall (WINAPI, CALLBACK) ?
4)	Какова структура минимального WinAPI - приложения ?
5)	Какие основные действия выполняются в главной функции програм - мы ?
...
10)	Какое значение должна вернуть оконная процедура, если для сооб - щения обработка не выполняется ?
...
void ex01()
{
	...
	*/
	...
	int hWord = (lParam >> 16) & 0xffff;
	int lWord = lParam & 0xffff;	
...
void ex04()
{
	...
	//
	// Вся необходимая информация для перерисовки окна будет представлена в 
	// структуре PAINTSTRUCT:
	//		
	//		struct PAINTSTRUCT {
