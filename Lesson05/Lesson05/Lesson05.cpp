...
void ex03()
{
	...
	*/
	/*
		Для получения дескриптора элемента управления по 
		его идентификатору  используется функция GetDlgltem:
		...
		Для получения идентификатора элемента управления по 
		его дескриптору используется функция API GetDlgCtrlID:
...
//Практическая часть
/*
	Создать диалоговое приложение, позволяющее перемещать и 
	пропорционально изменять размеры статика, расположенного 
	на форме диалога.
	
	Для перемещения статика следует использовать клавиши 
	управления курсором, а для пропорционального изменения 
	размеров статика – клавиши « + » и « - ».
*/
